...
router.post('/', async (req, res) => {
  try {
    ...
    // 👇 PEGA ACA
    if (payment.type === 'INCOME' && payment.patientId) {
      ...
      for (const debt of debts) {
        ...
        const amountToApply = Math.min(debt.remainingAmount, remainingPayment);

        const newPaid = (debt.paidAmount || 0) + amountToApply;
        const newRemaining = debt.remainingAmount - amountToApply;
